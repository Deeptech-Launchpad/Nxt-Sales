...
function signToken(user) {
  const requested = user?.settings?.security?.sessionTimeout
  const expiresIn = ['1d', '7d', '30d'].includes(requested) ? requested : '7d'
  return jwt.sign({ id: user.id, email: user.email, name: user.name, role: user.role }, SECRET, { expiresIn })
}

// POST /api/auth/demo
// Creates a real database-backed demo identity for local/staging previews so
// downstream routes that require a valid User foreign key continue to work.
// Production must opt in explicitly.
router.post('/demo', async (req, res) => {
  if (process.env.NODE_ENV === 'production' && process.env.ENABLE_DEMO_LOGIN !== 'true') {
    return res.status(404).json({ message: 'Demo workspace is not available.' })
  }

  try {
    const user = await prisma.user.upsert({
      where: { email: 'demo@altiusnxt.com' },
      update: { status: 'active', name: 'NXT Sales Demo' },
      create: {
        email: 'demo@altiusnxt.com',
        name: 'NXT Sales Demo',
        companyName: 'AltiusNxt Demo Workspace',
        role: 'admin',
        status: 'active',
      },
    })
    ...
    return res.json({
      ...
      user: { id: user.id, name: user.name, email: user.email, companyName: user.companyName, role: user.role, avatar: user.avatar },
    })
  } catch (err) {
    console.error('Demo login error:', err)
    return res.status(500).json({ message: 'Demo workspace is temporarily unavailable.' })
  }
})
...
router.post('/login', async (req, res) => {
  try {
    ...
    res.json({ success: true, token: signToken(user), user: { id: user.id, name: user.name, email: user.email, role: user.role, avatar: user.avatar } })
  } catch (err) {
    ...
    if (err.code === 'P1001' || err.message?.includes('Can\'t reach database')) {
      const devUser = {
        id: 'dev-user-001',
        name: req.body.email ? req.body.email.split('@')[0] : 'Demo User',
        email: req.body.email || 'user@altiusnxt.com',
        role: 'admin'
      }
      return res.json({ success: true, token: signToken(devUser), user: devUser })
    }
...
router.post('/register', async (req, res) => {
  try {
    ...
    if (!name || !email || !companyName || !password) return res.status(400).json({ message: 'All fields are required.' })

    const exists = await prisma.user.findUnique({ where: { email } })
    if (exists) return res.status(409).json({ message: 'Email already in use.' })

    const passwordHash = await bcrypt.hash(password, 10)
    const user = await prisma.user.create({ data: { name, email, companyName, passwordHash } })

    res.status(201).json({ success: true, token: signToken(user), user: { id: user.id, name: user.name, email: user.email, companyName: user.companyName, role: user.role, avatar: user.avatar } })
  } catch (err) {
    ...
    if (err.code === 'P1001' || err.message?.includes('Can\'t reach database')) {
      const devUser = {
        ...
        name: req.body.name || 'Prabakaran',
        email: req.body.email || 'prabakaranraja77@gmail.com',
        role: 'admin'
      }
      return res.status(201).json({ success: true, token: signToken(devUser), user: devUser })
    }
    res.status(500).json({ message: 'Server error.' })
  }
})
...
router.get('/google', (req, res, next) => {
  if (!process.env.GOOGLE_CLIENT_ID || !process.env.GOOGLE_CLIENT_SECRET) {
    return res.status(400).send('Google OAuth is not configured. Please add GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET in server/.env file.')
  }
...
router.get('/me', require('../middleware/authMiddleware'), async (req, res) => {
  ...
  res.json({ id: req.user.id, name: req.user.name, email: req.user.email, role: req.user.role || 'admin', avatar: null, status: 'active', createdAt: new Date() })
})
