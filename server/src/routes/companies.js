...
// Spreadsheet-friendly boolean parser for fields such as "Is Pinned".
// Blank/unknown text is treated as false for a newly-created record.
function importBoolean(v) {
  if (typeof v === 'boolean') return v
  if (typeof v === 'number') return v !== 0
  return ['true', 'yes', 'y', '1', 'pinned'].includes(String(v || '').trim().toLowerCase())
}
...
function dateRangeFor(key) {
  ...
  // Explicit calendar values used by the Companies date picker.
...
async function buildCompanyWhere(query, userId) {
  ...
  if (website) advancedAnd.push({ OR: [
    { domain:    { contains: website, mode: 'insensitive' } },
    { endPdpUrl: { contains: website, mode: 'insensitive' } },
  ] })
  if (updatedDate) {
    const range = dateRangeFor(updatedDate)
    if (range) advancedAnd.push({ updatedAt: { gte: range[0], lte: range[1] } })
  }
  if (lastActivity) {
    const range = dateRangeFor(lastActivity)
    if (range) advancedAnd.push({ activities: { some: { createdAt: { gte: range[0], lte: range[1] } } } })
  }
  if (advancedAnd.length) where.AND = advancedAnd
...
router.get('/', auth, async (req, res) => {
  try {
    const { page = 1, limit = 100, sortBy, sortDir } = req.query
    const where = await buildCompanyWhere(req.query, req.user.id)
    const sortableFields = new Set(['name', 'email', 'phone', 'industry', 'leadStatus', 'status', 'domain', 'country', 'cms', 'remarks', 'createdAt', 'updatedAt'])
    const orderBy = sortBy && sortableFields.has(sortBy)
      ? [{ isPinned: 'desc' }, { [sortBy]: sortDir === 'asc' ? 'asc' : 'desc' }]
    ...
    res.json({ companies: [], total: 0, page: Number(req.query.page || 1) })
  }
})

// ── GET /api/companies/summary ───────────────────────────
// Lightweight workspace metrics for the Companies overview. Kept separate
// from the paginated list so filters and paging never change these totals.
router.get('/summary', auth, async (_req, res) => {
  try {
    const base = { deletedAt: null }
    const [total, withDeals, unassigned, withCms] = await Promise.all([
      prisma.company.count({ where: base }),
      prisma.company.count({ where: { ...base, deals: { some: {} } } }),
      prisma.company.count({ where: { ...base, ownerId: null } }),
      prisma.company.count({
        where: {
          ...base,
          AND: [
            { cms: { not: null } },
            { cms: { notIn: ['', '--', 'N/A', 'n/a', 'None', 'none', 'Static', 'Less Data', 'No Data'] } },
          ],
        },
      }),
    ])
    res.json({ total, withDeals, unassigned, withCms })
  } catch (err) {
    console.error('Companies summary error:', err)
    res.status(500).json({ message: 'Unable to load company summary.' })
  }
})
...
router.get('/filter-options/cms', auth, async (_req, res) => {
  try {
    const rows = await prisma.company.findMany({
      where: { deletedAt: null, cms: { not: null } },
      select: { cms: true },
      distinct: ['cms'],
      orderBy: { cms: 'asc' },
    })
    ...
    const options = rows.flatMap(row => {
      const value = String(row.cms || '').trim()
      const key = value.toLowerCase()
      const placeholder = /^(--|n\/?a|none|null|static|less data|no data)$/i.test(value)
      const numericOnly = /^\d+(?:\.\d+)?$/.test(value)
      const operationalNote = /^(allocated|assigned)\s+to\b/i.test(value)
      if (!value || value.length > 80 || placeholder || numericOnly || operationalNote || seen.has(key)) return []
      seen.add(key)
      ...
      try {
        ...
        const newCompany = await prisma.company.create({
          data: {
            ...
            isPinned:              importBoolean(c.isPinned),
          },
        })
        created++
...
// ── PATCH /api/companies/:id/favorite — toggle Favorite star ────────────
router.patch('/:id/favorite', auth, async (req, res) => {
  try {
    const { id } = req.params
    const existing = await prisma.company.findUnique({ where: { id }, select: { deletedAt: true, isFavorite: true } })
    if (!existing || existing.deletedAt) return res.status(404).json({ message: 'Company not found.' })
