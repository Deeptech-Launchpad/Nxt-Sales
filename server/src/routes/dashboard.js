...
// GET /api/dashboard/next-actions — the Smart Priority Queue on the Main
// Dashboard. A compact, explainable ranking: every score is derived from CRM
// facts (pipeline progress, engagement, overdue work, data readiness) via
// leadScoring.js, never estimated, so each recommendation can show why it
// ranked where it did.
...
// POST /api/dashboard/follow-up-sequences — "Start follow-up" on a Smart
// Priority Queue item. Creates the enrollment and runs it once immediately
// (via the same processor the background sweep uses) so the first task
// appears right away instead of waiting up to 15 minutes for the next sweep.
