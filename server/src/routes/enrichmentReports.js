...
// Screenshot analysis is the slowest thing this app asks of Gemini — two
// full-page images plus a long prompt — so it sets its own deadline rather
// than the service default, which was chosen for short text prompts.
const GEMINI_TIMEOUT_MS = 120000

// Every Gemini call in this file goes through services/geminiService, which is
// the one place the API key is read and the one place usage is recorded. The
// request shape below is exactly what the removed axios transport already
// built, so the payload Google receives is unchanged; what changes is that
// these calls are now attributable on the AI Usage page and inherit the
// service's model-fallback chain instead of pinning a single model name.
const generateOnce = async (request, { feature, userId } = {}) => {
  const body = await gemini.generate(
    { contents: [{ parts: request.contents }], generationConfig: request.config },
    { feature, userId, timeoutMs: GEMINI_TIMEOUT_MS }
  )
...
const generateWithRetry = async (request, options = {}, attempts = 4) => {
  let lastError
  for (let attempt = 1; attempt <= attempts; attempt++) {
    try {
      return await generateOnce(request, options)
    } catch (error) {
      lastError = error
...
Score each category using this explicit 100-point rubric: presence/completeness 40 points, structure/consistency 25, specificity 20, buyer usefulness 15. Scores must be grounded only in visible evidence and include an evidence note. Use 0 when an area is not visible; never invent a value.`
    ...
    const aiCall = { feature: 'enrichment.analysis', userId: req.user?.id || null }
    ...
    const response = await generateWithRetry(analysisRequest, aiCall)
...
- Before responding, recount the visible rows and confirm internally that the JSON array contains the same number of rows.`
    ...
    const specificationResponse = await generateWithRetry(specificationRequest, { ...aiCall, feature: 'enrichment.specifications' }, 3)
...
Return ONLY the refined text as a plain string. Do not include markdown code fences or quote marks.`
    ...
    const response = await generateWithRetry(
      { contents: [{ text: prompt }], config: { temperature: 0.2 } },
      { feature: 'enrichment.refine', userId: req.user?.id || null },
      3
    )
