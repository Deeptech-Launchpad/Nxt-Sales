// Manage -> Settings. Everything here is scoped to req.user.id — there is no
// route that reads or writes another user's row, so this adds no new
// administrative surface.
//
// Preferences live in the single User.settings JSON column rather than a table
// per section: they are read and written whole, only ever by their owner, and
// are never queried across users. normalizeSettings() merges whatever is stored
// over DEFAULTS on every read, so a row written before a new key existed still
// comes back complete and no backfill migration is needed when one is added.
//
// The Gmail state exposed by GET / is read-only (an EmailAccount lookup). The
// connect/disconnect actions the UI offers are the existing
// /api/email/gmail/* endpoints — the Email module is not touched from here.
