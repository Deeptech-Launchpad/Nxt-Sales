  ...
  try {
    ...
    if (search && search.trim()) {
      ...
      conditions.push(Prisma.sql`(
        subject ILIKE ${q}
        OR "fromEmail" ILIKE ${q}
        OR "toEmail" ILIKE ${q}
        OR EXISTS (
          SELECT 1 FROM "Company" company_search
          WHERE company_search.id = "Activity"."companyId"
            AND company_search.name ILIKE ${q}
        )
      )`)
    }
...
// GET /api/email/import-fields — column list for the Inbox bulk-import
// template/mapping (DataImportModal). Deliberately small: this isn't for
// syncing real mail, it's for logging historical conversations that
// happened outside the system (e.g. a rep's own mailbox before this CRM was
// connected) so they show up alongside real synced/sent threads.
...
// POST /api/email/import — bulk-create logged (not sent) email Activity rows.
// A logged row only shows up in GET /inbox if its From/To address matches a
// known Company/Deal email or domain (the same EXISTS check every other
// inbox row must pass — see getCrmAddresses() above) — otherwise it would be
// saved but silently invisible, which is worse than refusing it outright, so
// rows that don't match are reported back as failures instead of created.
router.post('/import', auth, async (req, res) => {
  try {
    ...
    const { emails: crmEmails, domains: crmDomains } = await getCrmAddresses()
    // fromEmail/toEmail aren't always bare addresses — synced messages store
    // the raw Gmail header (e.g. `"John Roberts" <johnr@x.com>`, sometimes
    // several comma-separated recipients), same as every real inbox row.
    // extractAddresses() (used by the sync/dedup logic above) pulls the
    // actual address(es) out of that regardless of wrapping — matching
    // exactly what the /inbox EXISTS check does, so an address that's
    // already good enough to appear as a real conversation is also good
    // enough here.
    const isKnownAddress = (raw) => {
      for (const a of extractAddresses(raw)) {
        if (crmEmails.has(a)) return true
        const domain = a.split('@')[1]
        if (domain && crmDomains.has(domain)) return true
      }
      return false
    }
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const fromEmail = (r.fromEmail || '').trim()
      const toEmail   = (r.toEmail || '').trim()
      const subject   = (r.subject || '').trim()
      const dirRaw    = (r.direction || '').trim().toLowerCase()
      const direction = ['sent', 'outbound'].includes(dirRaw) ? 'outbound'
        : ['received', 'inbound'].includes(dirRaw) ? 'inbound'
        : null

      if (!subject)              { errors.push(`Row ${rowNum}: Subject is required.`); continue }
      if (!fromEmail && !toEmail) { errors.push(`Row ${rowNum}: From or To email is required.`); continue }
      if (!direction)            { errors.push(`Row ${rowNum}: Direction must be "Sent" or "Received".`); continue }
      if (!isKnownAddress(fromEmail) && !isKnownAddress(toEmail)) {
        errors.push(`Row ${rowNum}: Neither address matches an existing Company or Deal, so it would never show up in Inbox — skipped.`)
        continue
      }
    ...
    console.error('[Email Import] error:', err.message)
