...
// Avatar is stored in the existing User.avatar field. The cover image and
// cover quote live inside User.settings so this feature needs no schema
// migration and remains isolated per user.
router.get('/me/personalization', auth, async (req, res) => {
  try {
    const user = await prisma.user.findUnique({
      where: { id: req.user.id },
      select: { avatar: true, settings: true },
    })
    if (!user) return res.status(404).json({ message: 'Account not found.' })
    const settings = user.settings && typeof user.settings === 'object' && !Array.isArray(user.settings) ? user.settings : {}
    const personalization = settings.personalization && typeof settings.personalization === 'object'
      ? settings.personalization
      : {}
    res.json({
      avatar: user.avatar || '',
      coverImage: personalization.coverImage || '',
      coverQuote: personalization.coverQuote || '',
    })
  } catch (err) {
    console.error('Profile personalization fetch error:', err)
    res.status(500).json({ message: 'Unable to load profile personalization.' })
  }
})

router.put('/me/personalization', auth, async (req, res) => {
  try {
    const avatar = req.body.avatar === undefined ? undefined : String(req.body.avatar || '')
    const coverImage = req.body.coverImage === undefined ? undefined : String(req.body.coverImage || '')
    const coverQuote = req.body.coverQuote === undefined ? undefined : String(req.body.coverQuote || '').trim()

    if (avatar && !avatar.startsWith('data:image/')) return res.status(400).json({ message: 'Profile photo must be an image.' })
    if (coverImage && !coverImage.startsWith('data:image/')) return res.status(400).json({ message: 'Cover must be an image.' })
    if (avatar && avatar.length > 8_000_000) return res.status(413).json({ message: 'Profile photo is too large.' })
    if (coverImage && coverImage.length > 12_000_000) return res.status(413).json({ message: 'Cover image is too large.' })
    if (coverQuote !== undefined && coverQuote.length > 180) return res.status(400).json({ message: 'Cover quote must be 180 characters or fewer.' })

    const current = await prisma.user.findUnique({ where: { id: req.user.id }, select: { settings: true, avatar: true } })
    if (!current) return res.status(404).json({ message: 'Account not found.' })
    const settings = current.settings && typeof current.settings === 'object' && !Array.isArray(current.settings) ? current.settings : {}
    const existingPersonalization = settings.personalization && typeof settings.personalization === 'object'
      ? settings.personalization
      : {}
    const personalization = {
      ...existingPersonalization,
      ...(coverImage !== undefined ? { coverImage } : {}),
      ...(coverQuote !== undefined ? { coverQuote } : {}),
    }
    ...
    res.json({
      avatar: user.avatar || '',
      coverImage: user.settings?.personalization?.coverImage || '',
      coverQuote: user.settings?.personalization?.coverQuote || '',
    })
  } catch (err) {
    console.error('Profile personalization update error:', err)
    res.status(500).json({ message: 'Unable to save profile personalization.' })
  }
})
