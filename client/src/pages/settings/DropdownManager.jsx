...
// Every dropdown field this system manages, grouped for the left nav. Adding
// a new managed field elsewhere in the app means adding one entry here —
// the backend (GET /api/dropdowns) already returns whatever fieldKeys exist.
const FIELD_GROUPS = [
  {
    group: 'Company',
    fields: [
      { key: 'company.industry',   label: 'Industry' },
      { key: 'company.country',    label: 'Country (shared with Deal)' },
      { key: 'company.leadStatus', label: 'Lead Status' },
    ],
  },
  {
    group: 'Deal',
    fields: [
      ...
      { key: 'deal.clientType',           label: 'Client Type' },
      { key: 'deal.serviceRequirement',   label: 'Service Requirement' },
      { key: 'deal.opportunityType',      label: 'Opportunity Type' },
      { key: 'deal.strategicImportance',  label: 'Strategic Importance' },
      { key: 'deal.expectedOutcome',      label: 'Expected Outcome' },
    ],
  },
]
...
export default function DropdownManager() {
  ...
  const [selectedKey, setSelectedKey] = useState(FIELD_GROUPS[0].fields[0].key)
              ...
              {g.fields.map(f => (
                <button
                  ...
                  style={{
                    ...
                    color: selectedKey === f.key ? '#1d4ed8' : '#334155',
