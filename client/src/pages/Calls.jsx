...
// Export dropdown for the Calls dashboard — same exportUtils / CSV-Excel-
// JSON-PDF choices as Companies/Deals, styled to match this page's existing
// .ch-btn look (its own bespoke class, not the shared .btn-action other
// list pages use).
function CallsExportMenu({ fetchAllForExport }) {
  ...
  const run = async (fn, extraArgs = []) => {
    ...
    try {
      ...
      fn(records, 'calls', CALLS_EXPORT_COLUMNS, ...extraArgs)
    } catch {
  ...
  return (
    <div style={{ position: 'relative' }} ref={ref}>
      <button className="ch-btn ch-btn-outline" onClick={() => setOpen(o => !o)} disabled={exporting}>
        <Download size={13} /> {exporting ? 'Exporting…' : 'Export'} <ChevronDown size={11} />
      </button>
      {open && (
        <div style={{ position: 'absolute', top: 'calc(100% + 4px)', right: 0, background: '#fff', border: '1px solid #e2e8f0', borderRadius: 8, boxShadow: '0 8px 24px rgba(0,0,0,.12)', zIndex: 3000, minWidth: 170, overflow: 'hidden' }}>
          {[
            ...
            { label: 'Export as Excel', fn: () => run(exportXLSX, ['Calls']) },
            { label: 'Export as JSON',  fn: () => run(exportJSON) },
            { label: 'Export as PDF',   fn: () => run(exportPDF, ['Calls Export — NXT Sales']) },
          ].map(item => (
...
const CALL_TD_STYLE = {
  ...
  fromNumber:         { whiteSpace: 'nowrap', fontFamily: 'monospace', fontSize: 11 },
  toNumber:           { whiteSpace: 'nowrap', fontFamily: 'monospace', fontSize: 11 },
    ...
    switch (f.key) {
      ...
      case 'direction':
        return log.direction === 'inbound' ? (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, color: '#7c3aed', fontSize: 11, fontWeight: 600 }}>
            <PhoneIncoming size={12} /> Inbound
          </span>
        ) : log.direction === 'missed' ? (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, color: '#dc2626', fontSize: 11, fontWeight: 600 }}>
            <PhoneMissed size={12} /> Missed
          </span>
        ) : (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, color: '#2563eb', fontSize: 11, fontWeight: 600 }}>
  ...
  return (
    <>
      <style>{`
        ...
        .ch-table th { background: #f8fafc; font-size: 10px; font-weight: 700; color: #64748b; text-transform: uppercase; letter-spacing: .5px; padding: 8px 12px; text-align: left; border-bottom: 1px solid #e2e8f0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .ch-table td { padding: 9px 12px; font-size: 11.5px; color: #374151; border-bottom: 1px solid #f1f5f9; vertical-align: middle; overflow: hidden; text-overflow: ellipsis; }
        .ch-table tr:hover td { background: #fafbff; }
        .ch-btn { display: inline-flex; align-items: center; gap: 5px; padding: 5px 11px; border-radius: 6px; border: none; cursor: pointer; font-size: 11px; font-weight: 600; font-family: inherit; }
      ...
      <div style={{ padding: '24px 28px', fontFamily: 'DM Sans, sans-serif' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <div>
            <h1 style={{ fontSize: 17, fontWeight: 700, color: '#0f172a', margin: 0 }}>Calls Dashboard</h1>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            {syncMsg && (
              <span style={{ fontSize: 11, color: syncMsg.includes('failed') || syncMsg.includes('Failed') ? '#ef4444' : '#16a34a' }}>
                {syncMsg}
              </span>
            )}
            ...
            <button className="ch-btn ch-btn-outline" onClick={() => setShowImport(true)} title="Log a call made outside CallHippo">
              <Upload size={12} /> Import
            </button>
            <CallsExportMenu fetchAllForExport={fetchAllForExport} />
            ...
            <a
              ...
              style={{ textDecoration: 'none' }}
            >
              <Phone size={12} /> Open CallHippo <ExternalLink size={10} />
            </a>
          </div>
        </div>

        {/* Filters */}
        <div className="filter-chips-row" style={{ marginBottom: 14 }}>
                ...
                <tbody>
                  {logs.map((log, idx) => (
                    <tr key={log.id} style={{ background: selectedIds.has(log.id) ? '#f0f7ff' : undefined }}>
                      ...
                      <td style={{ color: '#94a3b8', fontSize: 11, fontWeight: 500, minWidth: 32 }}>
      ...
      <DataImportModal
        ...
        entityLabel="Calls"
        fieldsUrl="/callhippo/import-fields"
        importUrl="/callhippo/import"
        payloadKey="calls"
        templateName="calls_log"
      />
    </>
  )
}
