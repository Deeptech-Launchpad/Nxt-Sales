...
import { loadPersonalization, savePersonalizationLocal } from '../utils/personalization'
...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    api.get('/auth/me')
      .then(account => {
        if (!alive) return
        if (account?.data) {
          setUser(current => ({ ...current, ...account.data }))
          setAvatar(account.data.avatar || '')
        }
        const local = loadPersonalization(authUser?.id)
        if (local.avatar) setAvatar(local.avatar)
        setCoverImage(local.coverImage || '')
        setCoverQuote(local.coverQuote || '')
      })
      .catch(() => {})
  ...
  const savePersonalization = async () => {
    setSaving(true)
    try {
      const stored = savePersonalizationLocal(authUser?.id, { avatar, coverImage, coverQuote })
      if (!stored) {
        // Almost always the 5MB quota, hit by a large cover image.
        showNotice('Could not save — try a smaller cover image.', 'error')
        return
      }
      updateUser({ avatar: avatar || null })
      setUser(current => ({ ...current, avatar: avatar || null }))
      showNotice('Your profile style has been saved on this device.')
    } catch (error) {
