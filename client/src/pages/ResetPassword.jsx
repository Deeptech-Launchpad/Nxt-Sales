...
// Reuses /api/auth/validate-invite and /api/auth/accept-invite — the same
// token-with-expiry "verify then set a password" mechanism already built for
// invited users covers a password reset exactly as well; only the copy here
// is reset-specific, not the underlying request logic.
export default function ResetPassword() {
  ...
  useEffect(() => {
    ...
    api.get(`/auth/validate-invite?token=${token}`)
      .then(r => { setResetUser(r.data.user); setLoading(false) })
      .catch(() => {
    ...
    <div style={{
      ...
      <div style={{ background: '#fff', borderRadius: 16, padding: 36, width: '100%', maxWidth: 420, boxShadow: '0 24px 64px rgba(0,0,0,.3)' }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <div style={{ fontSize: 16, color: '#344054', marginTop: 2 }}>Set a new password</div>
        </div>
        ...
        {!loading && tokenError && (
          <div>
            ...
            <button onClick={() => navigate('/forgot-password')} style={{ width: '100%', background: '#0f172a', color: '#fff', border: 'none', borderRadius: 8, padding: 12, fontSize: 17, fontWeight: 600, cursor: 'pointer' }}>
              Request a new link
            </button>
          </div>
        )}
        ...
        {!loading && !tokenError && success && (
          <div style={{ textAlign: 'center', padding: '16px 0' }}>
            ...
            <h3 style={{ margin: '0 0 8px', color: '#0f172a', fontSize: 21 }}>Password updated!</h3>
            <p style={{ color: '#344054', fontSize: 16, margin: 0 }}>Redirecting you to the dashboard…</p>
          </div>
        )}

        {!loading && !tokenError && !success && resetUser && (
          <>
            <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 8, padding: '12px 14px', marginBottom: 22 }}>
              <p style={{ margin: 0, color: '#475569', fontSize: 16 }}>
                Set a new password for <strong style={{ color: '#0f172a' }}>{resetUser.email}</strong>.
              </p>
            </div>
