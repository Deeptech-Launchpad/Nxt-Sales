...
// Screenshot comparison runs SERVER-SIDE, via /enrichment-reports/analysis/generate.
//
// This previously branched on an `ai_key` read from localStorage and, when one
// was present, called Google directly from the browser through
// callGeminiWithFallback. That export no longer exists: the Gemini key was
// centralised onto the server so a paid key is never exposed to the browser,
// and nothing writes ai_key to localStorage any more — so that branch was
// already unreachable. The server route holds the key and returns exactly the
// parsed shape this function used to assemble by hand, so the browser-side
// prompt and JSON parsing are removed rather than left as dead code.
async function analyzeScreenshots(product) {
  const { data } = await api.post('/enrichment-reports/analysis/generate', {
