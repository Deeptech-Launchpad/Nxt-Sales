...
function Gauge({ value, label, color }) {
  return (
    <div style={{ textAlign: 'center', minWidth: 120 }}>
      ...
      <div style={{ fontSize: 14, color: '#344054', marginTop: 4, textTransform: 'uppercase', letterSpacing: 0.4 }}>{label}</div>
    </div>
  )
}
...
function AuthRow({ name, ok, detail }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 0' }}>
      <span style={{ width: 18, height: 18, borderRadius: '50%', background: s.bg, color: s.color, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700, flexShrink: 0 }}>{ok ? '✓' : '—'}</span>
      <strong style={{ fontSize: 15.5, width: 58 }}>{name}</strong>
      <span style={{ fontSize: 15, color: '#475569' }}>{ok ? (detail || 'Found') : 'Not detected'}</span>
    </div>
  )
}
...
export default function DeliverabilityReport({ open, analyzing, report, currentSubject, onClose, onSend, onApplyAI, sending }) {
  ...
  const sectionTitle = { fontSize: 15, fontWeight: 700, color: '#0f172a', textTransform: 'uppercase', letterSpacing: 0.5, margin: '18px 0 8px' }
    ...
    <div style={overlay} onMouseDown={e => { if (e.target === e.currentTarget && !sending) onClose() }}>
      <div style={card}>
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontSize: 18, fontWeight: 700, color: '#0f172a' }}>📬 Deliverability Report</div>
            <div style={{ fontSize: 14.5, color: '#475467' }}>Pre-send quality check · guidance only, not a guarantee</div>
          </div>
          <button onClick={onClose} disabled={sending} style={{ border: 'none', background: 'none', fontSize: 20, color: '#475467', cursor: sending ? 'default' : 'pointer' }}>×</button>
        </div>

        {/* Body */}
        <div style={{ padding: '4px 20px 8px', overflowY: 'auto' }}>
          {analyzing ? (
            <div style={{ padding: '48px 0', textAlign: 'center', color: '#344054' }}>
            ...
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 28, padding: '18px 0 6px' }}>
                ...
                <div style={{ textAlign: 'center', minWidth: 120 }}>
                  <span style={{ display: 'inline-block', padding: '5px 14px', borderRadius: 20, fontWeight: 700, fontSize: 18, color: '#fff', background: RISK_COLOR[risk] }}>{risk}</span>
                  <div style={{ fontSize: 14, color: '#344054', marginTop: 6, textTransform: 'uppercase', letterSpacing: 0.4 }}>Spam risk</div>
                </div>
              </div>
              {content?.stats && (
                <div style={{ textAlign: 'center', fontSize: 14.5, color: '#475467', marginBottom: 4 }}>
                ...
                {content?.checks?.map((c, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', gap: 9, alignItems: 'flex-start', padding: '6px 8px', borderRadius: 7, background: s.bg }}>
                      <span style={{ width: 17, height: 17, borderRadius: '50%', background: s.color, color: '#fff', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700, flexShrink: 0, marginTop: 1 }}>{s.icon}</span>
              ...
              {auth?.available ? (
                <div style={{ background: '#f8fafc', borderRadius: 8, padding: '8px 12px' }}>
                  <div style={{ fontSize: 14.5, color: '#344054', marginBottom: 4 }}>Sending domain: <strong>{auth.domain}</strong> {auth.isFreeDomain && <span style={{ color: '#d97706' }}>(free mailbox provider)</span>}</div>
                  ...
                  <div style={{ fontSize: 13.5, color: '#475467', marginTop: 5 }}>Best-effort DNS lookup. DKIM selectors are private, so “not detected” may still be signed by your provider.</div>
                </div>
              ) : (
                <div style={{ fontSize: 15, color: '#475467' }}>{auth?.message || 'Authentication check unavailable.'}</div>
              )}
                ...
                <>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                    {auth.recommendations.map((r, i) => {
                      ...
                      return (
                        <div key={i} style={{ display: 'flex', gap: 8, fontSize: 15.5, color: '#334155' }}>
              ...
              {ai?.available ? (
                <div style={{ background: '#f0f9ff', border: '1px solid #e0f2fe', borderRadius: 8, padding: '10px 12px' }}>
                  {ai.issues?.length > 0 && (
                    <ul style={{ margin: '0 0 8px', paddingLeft: 18, fontSize: 15.5, color: '#334155' }}>
                  ...
                  {(ai.improvedBody || ai.improvedSubject) && (
                    <button
                      ...
                      disabled={applied}
                      style={{ padding: '6px 14px', fontSize: 15, fontWeight: 600, borderRadius: 6, border: 'none', cursor: applied ? 'default' : 'pointer', background: applied ? '#cbd5e1' : '#0369a1', color: '#fff' }}
                    >
                ...
                <div style={{ fontSize: 15, color: '#475467' }}>
        ...
        <div style={{ padding: '14px 20px', borderTop: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 14, color: '#475467' }}>You decide — review, edit, or send.</span>
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={onClose} disabled={sending} style={{ padding: '9px 16px', fontSize: 16, fontWeight: 600, borderRadius: 7, border: '1px solid #cbd5e1', background: '#fff', color: '#334155', cursor: sending ? 'default' : 'pointer' }}>Back to edit</button>
            <button onClick={onSend} disabled={analyzing || sending} style={{ padding: '9px 18px', fontSize: 16, fontWeight: 700, borderRadius: 7, border: 'none', background: (analyzing || sending) ? '#94a3b8' : (risk === 'High' ? '#dc2626' : '#0f172a'), color: '#fff', cursor: (analyzing || sending) ? 'default' : 'pointer' }}>
