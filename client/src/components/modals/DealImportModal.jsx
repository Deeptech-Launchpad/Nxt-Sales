...
// Deal bulk import (Update 1).
//
// Reuses the same building blocks as the Company importer — papaparse for CSV,
// SheetJS for Excel, the server's dynamic /import-fields list for the column
// map — but talks to the Deal endpoints (/deals/import-fields, /deals/bulk).
...
export default function DealImportModal({ isOpen, onClose, onSuccess }) {
  ...
  useEffect(() => {
    ...
    api.get('/deals/import-fields')
      ...
      .catch(() => setError('Could not load the Deal field list.'))
  }, [isOpen])
  ...
  const runImport = async () => {
    ...
    try {
      const { data } = await api.post('/deals/bulk', { deals: rows })
      setResult(data)
    ...
    <div style={OVERLAY} onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div style={SHEET}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '18px 22px', borderBottom: '1px solid #eef1f5' }}>
          ...
          <h2 style={{ fontSize: 17, fontWeight: 700, color: '#0f172a' }}>Import Deals</h2>
            ...
            <div style={{ marginTop: 14, fontSize: 13, color: '#334155' }}>
              ...
              {ignored.length > 0 && (
                <div style={{ fontSize: 12, color: '#92400e', background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 7, padding: '8px 11px', marginTop: 10 }}>
                  These columns matched no Deal field and will be ignored: {ignored.join(', ')}
                </div>
              )}
            </div>
          )}
          ...
          {result && (
            <div style={{ marginTop: 14 }}>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', fontSize: 13.5, color: '#166534', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 7, padding: '10px 12px' }}>
                ...
                <span><strong>{result.created}</strong> deal{result.created === 1 ? '' : 's'} imported{result.failed ? `, ${result.failed} failed` : ''}.</span>
              </div>
