    ...
    <div ref={ref} style={{ position: 'relative' }}>
      <button
        ...
        style={{
          ...
          fontSize: 17, color: value ? '#0f172a' : '#94a3b8', cursor: 'pointer',
      ...
      {open && (
        <div style={{
          ...
          <div style={{ maxHeight: 220, overflowY: 'auto' }}>
            {loading ? (
              <div style={{ padding: '12px 14px', fontSize: 16, color: '#475467', textAlign: 'center' }}>Searching…</div>
            ) : results.length === 0 ? (
              <div style={{ padding: '12px 14px', fontSize: 16, color: '#475467', textAlign: 'center' }}>No companies found</div>
            ) : results.map(c => (
              <div
                key={c.id}
                onClick={() => pick(c)}
              ...
              >
                <div style={{ fontSize: 16, fontWeight: 500, color: '#0f172a' }}>{c.name}</div>
              </div>
            ))}
          </div>
        </div>
